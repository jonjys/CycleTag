...
export function SellerBuilder() {
  ...
  const pilotUrl = `mailto:${contact.email.general}?subject=${encodeURIComponent("CycleTag Seller Kit pilot")}&body=${encodeURIComponent(`Hello Nytto Labs,\n\nI would like a quote for a seller packaging pilot.\nBrand: ${kit.seller}\nProduct / SKU: ${kit.product} / ${kit.sku}\nNumber of SKUs:\nMonthly packages:\nSales channel:\nLanguages / countries:\n\nPlease confirm scope, price and delivery before starting.`)}`;
  return <>
    <header className="seller-heading no-print"><div className="section-kicker">CYCLETAG / SELLER KITS</div><h1>Your package.<br /><em>Your next sale.</em></h1><p>Put a reorder card inside your next package. The buyer scans, sees the exact SKU and chooses one of your store links. No competing search results.</p><div className="seller-chips"><span>Free DIY tool</span><span>Your links stay yours</span><span>EN / SV / 中文</span></div></header>
    ...
    <div className="seller-warning"><strong>Before putting this in a package</strong><p>Check the sales channel’s rules for packaging inserts, external links and affiliate attribution. This tool is not an approved integration or partnership with any marketplace. CycleTag does not check stock, ownership or compatibility, and does not add its own affiliate ID to your links.</p><label className="seller-consent"><input type="checkbox" checked={approved} onChange={e => setApproved(e.target.checked)} required /><span>I may use these links, have checked the applicable channel rules, and understand that all card data is public.</span></label></div>
