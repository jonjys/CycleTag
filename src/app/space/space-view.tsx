...
function SharedSpace({ space, encoded }: { space: SpacePayload; encoded: string }) {
  ...
  async function share() {
    ...
    try {
      if (canShare) await navigator.share({ title: space.name, text: "Open our CycleTag refill space", url });
  ...
  return <>
    ...
    <section className="space-item-grid" aria-label="Items in this space">{space.items.map((item, index) => {
      ...
      return <article className="space-item" key={index}><div className="space-item-number">0{index + 1}<span>{marketplaceName(item.m)}</span></div><h2>{item.n}</h2><p className="space-query">{item.q}</p><dl><div><dt>Replacement cycle</dt><dd>Every {item.i} days</dd></div><div><dt>Next scheduled date</dt><dd>{formatDate(cycle.nextDue)}</dd></div></dl><a className="primary-button" href={buildEbayLink(item, campaign, "scan")} target="_blank" rel="nofollow sponsored noopener">Find on {marketplaceName(item.m)}<ExternalLink size={18} aria-hidden="true" /></a><Link className="space-detail-link" href={`/tag#d=${encodeTag(item)}`}>Details & calendar reminder <ArrowRight size={16} aria-hidden="true" /></Link></article>;
    })}</section>
    <p className="spaces-note">{validCampaignId(campaign) ? "Marketplace links are affiliate links. CycleTag may earn a commission at no extra cost to you. " : ""}Check the model, seller, price and delivery before buying. Compatibility and availability are not guaranteed.</p>
